...
export function AdminLoginForm() {
  const [state, formAction, isPending] = useActionState(loginAdminAction, { error: null as string | null });
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-slate-900 py-12 px-4 sm:px-6 lg:px-8">
      <div className="w-full max-w-md space-y-8 bg-slate-800 p-8 rounded-xl shadow-2xl border border-slate-700">
        ...
        <form className="mt-8 space-y-6" action={formAction}>
          ...
          {state?.error && <div className="text-red-400 bg-red-400/10 border border-red-400/20 p-3 rounded-md text-sm text-center font-medium">{state.error}</div>}
          ...
          <div>
            ...
            >
              {isPending ? "Authenticating..." : "Enter Factory"}
            </button>
          </div>
